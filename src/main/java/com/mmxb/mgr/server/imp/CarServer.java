...
public class CarServer extends BaseServer{
    ...
    public String rentCar(@ModelAttribute User user,@ModelAttribute CarRent carRent){
        if (validateUser(user.getPhoneName(),user.getPassword())){
            SqlSession session = openSession();
            ...
            Car car = null;
            shopExample.clear();
            shopExample.createCriteria().andPositionEqualTo(carRent.getPosition());
            ...
            order.setOutTime(carRent.getOutTime());
            order.setBackTime(carRent.getBackTime());
